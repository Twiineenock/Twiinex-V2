...
public class MirrorNodeJsonConverterImpl implements MirrorNodeJsonConverter<JsonNode> {
  ...
  @Override
  public @NonNull List<org.hiero.base.data.ContractLog> toContractLogs(@NonNull JsonNode node) {
    Objects.requireNonNull(node, "jsonNode must not be null");
    if (!node.has("logs")) {
      return List.of();
    }

    final JsonNode logs = node.get("logs");
    if (!logs.isArray()) {
      throw new IllegalArgumentException("Logs node is not an array: " + logs);
    }

    return jsonArrayToStream(logs)
        .map(
            n -> {
              try {
                final String address = n.get("address").asText();
                final String data = n.get("data").asText();
                final List<String> topics =
                    jsonArrayToStream(n.get("topics")).map(t -> t.asText()).toList();
                final Instant timestamp =
                    Instant.ofEpochSecond(
                        n.get("timestamp").isNumber()
                            ? n.get("timestamp").asLong()
                            : Long.parseLong(n.get("timestamp").asText().split("\\.")[0]));
                final String transactionHash = n.get("transaction_hash").asText();
                final int transactionIndex = n.get("transaction_index").asInt();
                final ContractId contractId =
                    n.has("contract_id") && !n.get("contract_id").isNull()
                        ? ContractId.fromString(n.get("contract_id").asText())
                        : null;
                final String rootAddress =
                    n.has("root_address") && !n.get("root_address").isNull()
                        ? n.get("root_address").asText()
                        : null;
                final Integer bloom =
                    n.has("bloom") && !n.get("bloom").isNull() ? n.get("bloom").asInt() : null;

                return new org.hiero.base.data.ContractLog(
                    address,
                    data,
                    topics,
                    timestamp,
