...
public class MirrorNodeClientImpl extends AbstractMirrorNodeClient<JsonNode> {
  ...
  @Override
  public @NonNull Page<org.hiero.base.data.ContractLog> queryContractLogs(
      final ContractId contractId) throws HieroException {
    Objects.requireNonNull(contractId, "contractId must not be null");
    final String path = "/api/v1/contracts/" + contractId + "/results/logs";
    final java.util.function.Function<JsonNode, List<org.hiero.base.data.ContractLog>>
        dataExtractionFunction = node -> jsonConverter.toContractLogs(node);
    return new RestBasedPage<>(
        objectMapper, restClient.mutate().clone(), path, dataExtractionFunction);
  }
}
