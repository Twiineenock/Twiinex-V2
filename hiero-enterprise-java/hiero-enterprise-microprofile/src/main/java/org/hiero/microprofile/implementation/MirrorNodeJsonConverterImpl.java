            ...
            n -> {
              try {
                ...
                final List<String> topics =
                    jsonArrayToStream(obj.getJsonArray("topics")).map(t -> t.toString()).toList();
                final Instant timestamp =
                    Instant.ofEpochSecond(
                        (long) Double.parseDouble(obj.getString("timestamp")));
                final String transactionHash = obj.getString("transaction_hash");
                final int transactionIndex = obj.getInt("transaction_index");
                final ContractId contractId =
                    obj.isNull("contract_id")
                        ? null
                        : ContractId.fromString(obj.getString("contract_id"));
                final String rootAddress =
                    obj.isNull("root_address")
                        ? null
                        : obj.getString("root_address");
                final Integer bloom =
                    obj.isNull("bloom") ? null : obj.getInt("bloom");
